...
{
    public static void ShowPieSeries(PieSeriesData stats, string title)
    {
        var model = new PlotModel { Title = "Китайский календарь" };
		
        var pieModel = new PieSeries 
		{ 
			StrokeThickness = 2.0, 
        ...
        foreach (var elem in stats.Data)
        {
            pieModel.Slices.Add(new PieSlice(elem.Key, elem.Value) { IsExploded = true});
        }


        model.Series.Add(pieModel);
